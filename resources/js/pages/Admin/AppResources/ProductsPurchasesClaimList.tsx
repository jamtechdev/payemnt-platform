...
interface Row { id: number; customer_email: string; product_code: string; date: string; description?: string; acknowledged: string; date_added?: string; partner?: { name?: string }; }
...
export default function ProductsPurchasesClaimList({ items, filters }: { items: Paginated; filters?: { search?: string } }) {
    const rows = items?.data ?? [];
    const [search, setSearch] = useState(filters?.search ?? '');
    const apply = () => router.get(route('admin.app-resources.products-purchases-claims'), { search }, { preserveState: true });
    ...
    const columns = [
        ...
        col.accessor((r) => fmtDate(r.date), { id: 'date', header: 'Claim Date' }),
        col.accessor((r) => r.description ?? '-', { id: 'description', header: 'Description' }),
        col.accessor('acknowledged', { header: 'Acknowledged', cell: (info) => <StatusBadge status={info.getValue()} type="customer" /> }),
        col.accessor((r) => fmtDate(r.date_added), { id: 'date_added', header: 'Date Added' }),
    ];

    return (
        <AdminLayout title="Purchase Claims">
            <Card className="mb-4"><CardContent className="pt-4"><div className="flex gap-3">
                <div className="relative flex-1"><Search className="absolute top-2.5 left-3 h-4 w-4 text-muted-foreground" />
                    <input className="w-full rounded-md border border-input bg-background py-2 pr-3 pl-9 text-sm" placeholder="Search email, product code..." value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && apply()} />
                </div><Button onClick={apply}>Search</Button>
            </div></CardContent></Card>
            <Card><CardHeader><CardTitle className="text-base">Purchase Claims</CardTitle></CardHeader>
                <CardContent>
                    <DataTable columns={columns} data={rows} showHeader showRowCount stripedRows clickableRows={false} emptyMessage="No claims found." stickyHeader compact />
