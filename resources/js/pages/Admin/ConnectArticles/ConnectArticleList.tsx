...
export default function ConnectArticleList({ articles, filters }: { articles: Paginated; filters?: Filters }) {
    const rows = articles?.data ?? [];
    const [form, setForm] = useState<Filters>(filters ?? {});

    const applyFilters = () => {
        router.get(route('admin.connect-articles.index'), form as Record<string, string>, { preserveState: true });
    };

    const columnHelper = createColumnHelper<ArticleRow>();
    const columns = [
        columnHelper.accessor((r) => r.article_code ?? '—', { id: 'article_code', header: 'Article Code' }),
        columnHelper.accessor((r) => r.title ?? '—', { id: 'title', header: 'Title' }),
        ...
        columnHelper.accessor((r) => r.from_platform ?? 0, {
            id: 'from_platform', header: 'From Platform',
            cell: (info) => (
        ...
        columnHelper.display({
            ...
            cell: (info) => (
                <ActionBtn tone="primary" href={route('admin.connect-articles.show', info.row.original.connect_articles_id)} title="View">
    ...
    return (
        <AdminLayout title="Connect Articles">
            <Card className="mb-4">
                    ...
                    <div className="grid gap-3 md:grid-cols-4">
                        <div className="relative md:col-span-2">
                            ...
                            <input
                                ...
                                placeholder="Search title, code, partner..."
                                value={form.search ?? ''}
            ...
            <div className="mb-4 grid gap-4 md:grid-cols-3">
                <Card className="border-blue-200/70 bg-blue-50/50">
                    ...
                    <CardContent className="text-2xl font-semibold">{articles.total ?? 0}</CardContent>
                </Card>
            ...
            <Card>
                <CardHeader><CardTitle className="text-base">Connect Article Records</CardTitle></CardHeader>
                <CardContent>
                    <DataTable columns={columns} data={rows} showHeader showRowCount stripedRows clickableRows={false} emptyMessage="No connect articles found." stickyHeader compact />
                    <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
                        <span>Showing {articles.from ?? 0}–{articles.to ?? 0} of {articles.total ?? 0}</span>
