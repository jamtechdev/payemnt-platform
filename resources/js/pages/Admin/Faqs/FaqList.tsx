...
export default function FaqList({ faqs, filters }: { faqs: Paginated; filters?: Filters }) {
    const rows = faqs?.data ?? [];
    const [form, setForm] = useState<Filters>(filters ?? {});

    const applyFilters = () => {
        router.get(route('admin.faqs.index'), form as Record<string, string>, { preserveState: true });
    };

    const columnHelper = createColumnHelper<FaqRow>();
    const columns = [
        columnHelper.accessor((r) => r.faq_code ?? '—', { id: 'faq_code', header: 'FAQ Code' }),
        columnHelper.accessor((r) => r.question ?? '—', {
        ...
        <AdminLayout title="FAQs">
                    ...
                    <div className="grid gap-3 md:grid-cols-4">
                        <div className="relative md:col-span-2">
                            ...
                            <input
                                ...
                                placeholder="Search question, code, partner..."
                                value={form.search ?? ''}
            ...
            <div className="mb-4 grid gap-4 md:grid-cols-3">
                <Card className="border-blue-200/70 bg-blue-50/50">
                    ...
                    <CardContent className="text-2xl font-semibold">{faqs.total ?? 0}</CardContent>
                </Card>
            ...
            <Card>
                <CardHeader><CardTitle className="text-base">FAQ Records</CardTitle></CardHeader>
                <CardContent>
                    <DataTable columns={columns} data={rows} showHeader showRowCount stripedRows clickableRows={false} emptyMessage="No FAQs found." stickyHeader compact />
                    <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
                        <span>Showing {faqs.from ?? 0}–{faqs.to ?? 0} of {faqs.total ?? 0}</span>
