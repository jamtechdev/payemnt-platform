...
export default function RateApiList({ rateApis, filters }: { rateApis: Paginated; filters?: Filters }) {
    const rows = rateApis?.data ?? [];
    const [form, setForm] = useState<Filters>(filters ?? {});

    const applyFilters = () => {
        router.get(route('admin.rate-apis.index'), form as Record<string, string>, { preserveState: true });
    };

    const columnHelper = createColumnHelper<RateApiRow>();
    const columns = [
        columnHelper.accessor((r) => r.name ?? '—', { id: 'name', header: 'Name' }),
        columnHelper.accessor((r) => r.url ?? '—', {
            id: 'url', header: 'URL',
            cell: (info) => <span className="max-w-xs truncate block text-xs font-mono">{info.getValue()}</span>,
        }),
        ...
        <AdminLayout title="Rate APIs">
                    ...
                    <div className="grid gap-3 md:grid-cols-4">
                        <div className="relative md:col-span-2">
                            ...
                            <input
                                ...
                                placeholder="Search name, partner..."
                                value={form.search ?? ''}
            ...
            <div className="mb-4 grid gap-4 md:grid-cols-3">
                <Card className="border-blue-200/70 bg-blue-50/50">
                    ...
                    <CardContent className="text-2xl font-semibold">{rateApis.total ?? 0}</CardContent>
                </Card>
            ...
            <Card>
                <CardHeader><CardTitle className="text-base">Rate API Records</CardTitle></CardHeader>
                <CardContent>
                    <DataTable columns={columns} data={rows} showHeader showRowCount stripedRows clickableRows={false} emptyMessage="No rate APIs found." stickyHeader compact />
                    <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
                        <span>Showing {rateApis.from ?? 0}–{rateApis.to ?? 0} of {rateApis.total ?? 0}</span>
                        <div className="flex gap-1">
                            {rateApis.links.map((link, i) => (
                                <button
                                    key={i}
                                    disabled={!link.url}
