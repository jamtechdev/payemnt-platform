...
function buildNavItems(auth: PageProps['auth']): NavItem[] {
    ...
    if (modules.includes('dashboard')) {
        if (role === 'super_admin') {
            ...
            items.push({ href: route('admin.reconciliation.dashboard'), label: 'Dashboard' });
    ...
    if (modules.includes('reports')) {
        if (permissions.includes('reports.customer_acquisition')) {
            items.push({ href: route('admin.reconciliation.reports.customer-acquisition'), label: 'Acquisition' });
        }
        if (permissions.includes('reports.revenue_by_product')) {
            items.push({ href: route('admin.reconciliation.reports.revenue'), label: 'Revenue' });
        }
        if (permissions.includes('reports.partner_performance')) {
            items.push({ href: route('admin.reconciliation.reports.partner-performance'), label: 'Partner performance' });
        }
    }
